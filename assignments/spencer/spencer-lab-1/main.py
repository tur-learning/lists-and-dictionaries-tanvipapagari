...
async def main():
    #defining some varibles for different colors, the parenthesis take RGB values
    BLACK = (  0,   0,   0)
    WHITE = (255, 255, 255)
    RED   = (255,   0,   0)
    GREEN = (  0, 255,   0)
...
#defining variables for directions of movement for the boxes and establishing the value for the initial movespeed
    DOWNLEFT  = 'downleft'
    DOWNRIGHT = 'downright'
    UPLEFT    = 'upleft'
    UPRIGHT   = 'upright'
...
#setting variables for the window height and width to use later for collision of edges
    WINDOWHEIGHT = 800
    WINDOWWIDTH = 800
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT:
                pygame.quit()
                sys.exit()
        #mainting the background color so the boxes do not leave a line
        windowSurface.fill(CUSTOM)
        #updating the position of the boxes
        for b in boxes:
            if b['dir'] == DOWNLEFT:
            ...
            #printing the collision with the top of box 2 the green one. It kinda works but the sides are funky
        ...
        #collision between box 1 and the left side of box 2
            ...
            if intersectionb1_2d:
                ...
                if b1['dir'] == DOWNRIGHT:
                    b1['dir'] = DOWNLEFT
                    print('poop it read the direction as downright')
                if b2['dir'] == UPLEFT:
                    b2['dir'] = UPRIGHT
                if b2['dir'] == DOWNLEFT:
                    b2['dir'] = DOWNRIGHT
...
#COLLISION BETWEEN BOX 2 AND BOX 3

            intersectionb2_3a = b2['rect'].clipline(b3['rect'].topleft,b3['rect'].topright)
            if intersectionb2_3a:
                if b2['dir'] == DOWNRIGHT:
                    b2['dir'] = UPRIGHT
                if b2['dir'] == DOWNLEFT:
                    b2['dir'] = UPLEFT
        ...
        #collision between box 2 and the bottom of box 3
            intersectionb2_3b = b2['rect'].clipline(b3['rect'].bottomleft,b3['rect'].bottomright)
            if intersectionb2_3b:
                if b2['dir'] == UPRIGHT:
                    b2['dir'] = DOWNRIGHT
                if b2['dir'] == UPLEFT:
                    b2['dir'] = DOWNLEFT
        ...
        #collision between box 2 and the right side of box 3
            intersectionb2_3c = b2['rect'].clipline(b3['rect'].topright,b3['rect'].bottomright)
            if intersectionb2_3c:
                if b2['dir'] == UPLEFT:
                    b2['dir'] = UPRIGHT
                    print('upleft collide')
                if b2['dir'] == DOWNLEFT:
        ...
        #collision between box 2 and the left side of box 2
            intersectionb2_3d = b2['rect'].clipline(b3['rect'].topleft,b3['rect'].bottomleft)
            if intersectionb2_3d:
                if b2['dir'] == UPRIGHT:
                    b2['dir'] = UPLEFT
                if b2['dir'] == DOWNRIGHT:
                    b2['dir'] = DOWNLEFT
